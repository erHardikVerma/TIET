...
R1=10;
A=(1+Rf/R1);
system = signal.lti(np.multiply(A,[2*pi*C,0.0]), [2*pi*R*C,1.0])
f = logspace(1, 5)
# w = 2 * pi * f
...
axs.plot([freq2,max(f)],[mag2,mag2],'--',linewidth=lw, color='r')
...
axs.legend(loc='lower right',ncol=5,fontsize=fnt_sz4)
axs.set_xlabel(r'$\omega$ (Hz)',fontsize=fnt_sz2)
axs.set_ylabel(r'Gain (dB)',fontsize=fnt_sz2)
axs.set_title('High Pass Filter',fontsize=fnt_sz2)
