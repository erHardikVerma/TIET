...
plt.sca(ax1)
plt.plot([freq[0],fc],[20.0*np.log10(max(mag))-20.0*np.log10(np.sqrt(2)),20.0*np.log10(max(mag))-20.0*np.log10(np.sqrt(2))],'r--')
plt.plot([fc,fc],[20.0*np.log10(min(mag)),20.0*np.log10(max(mag))-20.0*np.log10(np.sqrt(2))],'r--')
plt.tight_layout()
pp=PdfPages('low_pass_plot.pdf')
pp.savefig()
d = pp.infodict()
d['Title'] = 'Low Pass Filter Frequency Response'
